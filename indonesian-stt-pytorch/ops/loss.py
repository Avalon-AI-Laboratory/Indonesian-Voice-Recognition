...
class SmoothCTC_CrossEntropyLoss(nn.Module):
    ...
    def forward(self, model_output, dec_target):
        ...
        mask = dec_target != 0
        T, C = model_output.shape[1], model_output.shape[2]
        ...
        input_lengths = torch.full(size=(C, ), fill_value = T, dtype = torch.long).to(device)
        target_lengths = torch.sum(mask, dim = 1).to(device)
        ctc_loss = ctc_function(logits, dec_target, input_lengths, target_lengths)

        true_dist = torch.zeros_like(logits).to(device)
        true_dist.fill_(self.smoothing / (C - 1))
        true_dist.scatter_(2, dec_target.unsqueeze(2).to(device), 1 - self.smoothing)
        ce_smoothing_loss = (-true_dist * logits).sum(dim = 2)
        ce_smoothing_loss = (ce_smoothing_loss * mask.to(device))/sum() / mask.sum().to(device)
